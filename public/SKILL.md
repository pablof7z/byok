...
6. Store returned BYOK grants/tokens securely. A multi-provider grant returns `token_type: "raw_api_keys"` plus a `providers` array; save only the providers returned, since the user may skip unavailable or unwanted keys. Retrieve or proxy provider keys only when needed, and never print, log, commit, or persist raw provider API keys outside the approved secret store.
